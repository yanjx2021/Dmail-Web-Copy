        ...
        return (
            <div className="input-group mb-2">
                <input
                    ...
                    onChange={action((e) => { authStore.email = e.target.value })}
                />
            </div>
        )
    }
...
const LoginCodeForm = observer(
    ({ authStore }: { authStore: AuthStore }) => {
        return ((
            <div className="mb-4 mt-5">
                <EmailInput authStore={authStore} />
                <EmailCodeInput email={authStore.email} emailCode={authStore.emailCode} setEmailCode={action((code) => { authStore.emailCode = code })} />
            </div>
        ))
    }
)

const LoginPasswordForm = observer(
    ({ authStore }: { authStore: AuthStore }) => {
        return (
            <div className="mb-4 mt-5">
                <EmailInput authStore={authStore} />
                <div className="input-group mb-4">
                    <input
                        ...
                        onChange={action((e) => { authStore.password = e.target.value })}
                    />
                </div>
            </div>
        )
        ...
        return (
            <div className="card-body">
                ...
                <p className="text-center mb-6">
                    ...
                    <button
                        className="link"
                        onClick={authStore.toggleLoginMethod}>
