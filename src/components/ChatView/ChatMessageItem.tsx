...
export const ChatMessageItemContent = observer(
    ({ msg, scroll }: { msg: ChatMessage; scroll: Function }) => {
        ...
        if (msg.type === ChatMessageType.Text && typeof msg.content === 'string') {
            ...
            return (
                <div className={'message-content p-3' + (isRight ? ' border' : '')}>
                    ...
                    {msg.translatedText && (
                        <div className="quote-container">{msg.translatedText}</div>
                    )}
                </div>
            )
            ...
            const shortMessage = (operateMessage: ChatMessage) => (
                <p key={operateMessage.inChatId}>{`${
                    userStore.getUser(operateMessage.senderId).showName
                }: ${operateMessage.asShort.slice(
                    0,
            ...
            return (
                <div
                    id={`reply${msg.chatId}+${msg.inChatId}`}
                    className={'message-content p-3' + (isRight ? ' border' : '')}
                    onClick={action(() => {
